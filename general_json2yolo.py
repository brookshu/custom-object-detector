...
def convert_coco_json(json_dir='annotations/annotate2', use_segments=False, cls91to80=False):
    save_dir = make_dirs()  # output directory
    coco80 = coco91_to_coco80_class()
    print(json_dir)
    print(sorted(Path(json_dir).resolve().glob('*.json')))
    # Import json
    for json_file in os.listdir(json_dir): # check files in the folder
        print(json_file)
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            # Write
            file = Path((fn / f).with_suffix('.txt'))
...
if __name__ == '__main__':
    ...
    if source == 'COCO':
        convert_coco_json('annotate_output_correct',  # directory with *.json
                          use_segments=False,
                          cls91to80=False)
